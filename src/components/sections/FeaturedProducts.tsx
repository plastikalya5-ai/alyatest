...
export default function FeaturedProducts({ products }: { products: Product[] }) {
  return (
    <section id="products" className="bg-[#111511]" data-bg="#111511" style={{ paddingBlock:"clamp(72px,9vw,130px)" }}>
      <div style={{ paddingInline:"clamp(20px,5vw,80px)" }}>
        <div className="flex flex-wrap items-end justify-between gap-4 mb-12">
          <div>
            ...
            <h2 className="anim-split-heading heading text-[#eae6dd]" style={{ fontSize:"clamp(44px,7vw,96px)" }}>KOLEKSİYON</h2>
          </div>
          <a href="#collection" className="anim-up eyebrow text-[#6b7366] hover:text-white transition-colors">Tüm Ürünler →</a>
        </div>

        <div className="anim-stagger-parent grid grid-cols-2 md:grid-cols-4 gap-2">
          {products.map((p, i) => (
            <a key={p.id} href="#contact"
              className={`anim-stagger-child group relative overflow-hidden aspect-square ${i === 0 ? "md:col-span-2 md:row-span-2" : ""}`}

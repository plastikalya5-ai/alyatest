...
export default function Footer({ settings }: { settings: Settings | null }) {
  ...
  return (
    <footer className="bg-[#0b0e0b] border-t border-white/8" style={{ paddingTop:"clamp(56px,7vw,90px)", paddingBottom:"clamp(32px,4vw,48px)" }}>
      <div style={{ paddingInline:"clamp(20px,5vw,80px)" }}>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-x-10 gap-y-8">
            {cols.map(col => (
              <div key={col.title}>
                ...
                {col.items.map(item => (
                  <p key={item} className="text-[#6b7366] hover:text-white transition-colors text-sm mb-2.5 leading-snug">{item}</p>
                ))}
              </div>
            ))}
          </div>
        ...
        <div className="border-t border-white/8 pt-6 flex flex-wrap justify-between items-center gap-3">
          ...
          <p className="eyebrow text-white/20 text-[10px]">İstanbul OSB · Made in Türkiye 🇹🇷</p>
        </div>
      </div>
    </footer>
  );
